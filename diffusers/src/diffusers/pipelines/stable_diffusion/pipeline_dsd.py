...
class DSDPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromCkptMixin):
    r"""
    ...
    def __init__(
        ...
        elif random_noise_level is None:
            self.noise_samples = torch.randn(1000, 4, 64, 64)
            directory = os.path.expanduser("~/diffusers")
            if not os.path.exists(directory):
                os.makedirs(directory)
            with open(os.path.expanduser(f"~/diffusers/noise_samples.pkl"), "wb") as f:
                pickle.dump(self.noise_samples, f)
        else:
            noise_levels = [0.2, 0.4, 0.6, 0.8]
            self.noise_samples = torch.randn(1000, 4, 64, 64)
            for level in noise_levels:
                adjusted_noise = self.noise_samples * level
                with open(os.path.expanduser(f"~/diffusers/noise_samples_{level}.pkl"), "wb") as f:
                    pickle.dump(adjusted_noise, f)
    ...
    def sampled_timesteps(self, num_inference_steps, strength, sampling_steps, device):
    ...
    def __call__(
        ...
        layer_mask = None,
        unconditional = True,
        ublock: Optional[str] = None,
        sampled_timestep: Optional[str] = None, 
        level: Optional[str] = None,  
        block: Optional[str] = None,  
        attn: Optional[str] = None, 
        tfblock: Optional[str] = None, 
        ensembling = True,
    ):
        r"""
        ...
        prompt_embeds = self._encode_prompt(
            ...
            device,
            num_images_per_prompt,
            do_classifier_free_guidance or unconditional,
            negative_prompt,
        ...
        for sampling_step, t in enumerate(self.timesteps):
            ...
            noise_uncond, noise_cond, dbscore = self.unet(latent_model_input, t, noise, encoder_hidden_states=prompt_embeds, use_bias = use_bias, layer_mask = None, sampled_timestep = sampled_timestep, level = level, block = block, attn = attn, tfblock = tfblock)

            t_scores.append(dbscore)
            
            if do_classifier_free_guidance:
                noise_pred = noise_uncond + guidance_scale * (noise_cond - noise_uncond)

        if sampled_timestep:
            sampled_score = t_scores[sampled_timestep]
            stacked_scores = torch.stack(sampled_score)
            if stacked_scores.dim() == 1:
                ascore = stacked_scores.unsqueeze(1).permute(1, 0).mean(dim=1)
            else:
                ascore = stacked_scores.permute(1, 0).mean(dim=1)
        elif ensembling:
            stacked_scores = torch.stack(t_scores)
            if stacked_scores.dim() == 1:
                ascore = stacked_scores.unsqueeze(1).permute(1, 0).mean(dim=1)
            else:
                ascore = stacked_scores.permute(1, 0).mean(dim=1)
        else:
            sampled_score = t_scores[-1]
            stacked_scores = torch.stack(sampled_score)
            if stacked_scores.dim() == 1:
                ascore = stacked_scores.unsqueeze(1).permute(1, 0).mean(dim=1)
            else:
